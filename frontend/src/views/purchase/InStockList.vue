<template>
  <div class="page-container">
    <el-card>
      <div class="toolbar">
        <el-button v-permission="'purchase:instock:add'" type="primary" @click="handleAdd">新增入库</el-button>
      </div>
      <el-table :data="tableData" border stripe>
        <el-table-column prop="stock_no" label="入库单号" min-width="150" />
        <el-table-column prop="order_no" label="关联采购订单" min-width="150" />
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button v-permission="'purchase:instock:edit'" link type="primary" @click="handleEdit(row)">编辑</el-button>
            <el-button v-permission="'purchase:instock:delete'" link type="danger" @click="handleDelete(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="关联订单" prop="order">
              <el-select v-model="form.order" placeholder="请选择采购订单" filterable style="width: 100%">
                <el-option v-for="o in orderOptions" :key="o.id" :label="`${o.order_no} - ${o.supplier_name}`" :value="o.id" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
...
import { getInStockList, createInStock, updateInStock, deleteInStock } from '@/api/purchase'
import { getOrderOptions } from '@/api/purchase'
...
const rules = {
  ...
  order: [{ required: true, message: '请选择采购订单', trigger: 'change' }],
  stock_date: [{ required: true, message: '请选择入库日期', trigger: 'change' }],
}

const fetchData = async () => {
  const res = await getInStockList(query.value)
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (isEdit.value) {
        await updateInStock(currentId.value, payload)
        ElMessage.success('更新成功')
      } else {
        await createInStock(payload)
